"""Deferred hypothesis ranking by expected research value."""
...
MECHANISM_QUALITY = {
    "information_delay": 0.8,
    "risk_compensation": 0.8,
...
def score_hypothesis(h: ResearchHypothesis) -> dict:
    ...
    novelty = (
        1.0
        if not h.similar_previous_experiments
        else max(0.2, 1.0 - 0.2 * len(h.similar_previous_experiments))
    )
    info_gain = min(1.0, 0.4 + 0.3 * len(h.feature_groups))
    mechanism = MECHANISM_QUALITY.get(h.expected_mechanism, 0.5)
    duplicate_risk = min(1.0, fails * 0.25)
    cost = 0.2 + 0.05 * len(h.feature_groups)
...
def rank(hypotheses: list[ResearchHypothesis], min_score: float = 0.0) -> list[dict]:
    ranked = sorted(
        (score_hypothesis(h) for h in hypotheses),
